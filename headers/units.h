...
!    3 = equilibrium
!   -1 = single gamma law
!    Set ac and bc for mixture (ac: para component, bc: ortho component)
!    also pick a metallicity by adjusting xabun, yabun, and zabun.
      ...
      PARAMETER(torp=54.2d0,cct=3.0d0,cq=3d0,tcoollmt=14d0,theatlmt=5d1)
      PARAMETER(ictype=0,irtype=0,jcool=2)
!      PARAMETER(ictype=1,irtype=0,jcool=50)
      PARAMETER(jirr=10,irop=1.,Tbgrnd=3d0)
      PARAMETER(tenvk=50d0,fk=1d0)
      PARAMETER(SETUNITS=0,H2STAT=-1) 
      logical,parameter::find_eos_rho=.true.
      PARAMETER(amp0=0.0001d0) ! amplitude of initial, random perturbation
