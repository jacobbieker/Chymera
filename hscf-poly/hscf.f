...
c     prmtr.h contains the grid dimensions
      ...
      parameter (delta=1.0e-08, rho0=1.0d0,
...
c
      ...
      if (case.eq.1) then
        ...
        if(np.eq.100) read(5,*) xcut,xwidth,xcore
c
c     np is nprime (for  infinity, enter a value .ge. 3.0 .and. .le. 10.0);
c           nprime -- 100 ===> accreting star 
...
!
 35   continue
      ...
      do j=2,jmax1
        write(97,199) j,rhf(j),rho(j,2),(h(j,2)/(1.0+pin))**pin,
     &          h(j,2),hmax(n)
 199    format(i8,1p5e12.4)
      enddo
!
!
...
c     converts to cgs units
   ...
   80 continue
     ...
     1              sow,tow,vc
      write(12,110) rb,h0sq(nfin),tmss,tt,-ww,-ws,3.0*ss,starm
...
c     normalize the mass
      ...
      if (np .gt. 3.0d0 .and. np. le. 10.0) then
        call  inomega(np,omega,xmas,rhf,jout,jmax1)
      elseif (np .le. 3.0d0 .or. np .eq. 100.0) then
...
c  specific angular momentum distribution

         jr(j)    = f(q,np,rad)
         omega(j) = jr(j)/rad**2

c  power law angular velocity distribution 
...
c
c
c  accreting white dwarf dwarf angular velocity distribution
c
      do j =2,jout
        q = xmas(j)
        if (q.le.xcut) jcut=j
      ...
      do j =2,jout
         ...
         omega(j) = xcore+rhf(jcut)**2/(fermi+rad**2)
      end do
c
c
      return
