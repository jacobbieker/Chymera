...
#include "globals.h"
      character(len=*),parameter :: fmt="(a,1x,i3,1x,i3,1x,i3,1x,f9.3)"
      integer jstart
c      VLIM=two*SOUND
       VLIM=15.d0*SOUND
      if (jmin.gt.2) then
...
!$OMP DO SCHEDULE(STATIC)
        DO l=1,lmax,1
          DO k=2,kmax1,1
            DO j=jstart,jmax1,1
            SPEED1=sqrt(gamma*p(j,k,l)/(rho(j,k,l)))
...
!            if(rho(j,k,l)<100*rholmt) then
!             DELT3=DELT1
!            else
            DELT3=rhf(j)*DTHETA/(SPEED1+SPEED4+speed7)
!            endif
            !print *, j,k,l,delt1,delt2,delt3
